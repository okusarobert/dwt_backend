            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15), auth=self.auth) as session:
                async with session.post(url, json=payload) as response:
                    ...
                    if response.status == 200:
                        ...
                        if 'error' in data:
                            print("   ❌ RPC getaddresshistory failed with error: {}".format(data['error']))
                            return False
                        elif 'result' in data:
                            print("   ✅ RPC getaddresshistory successful")
                            return True
                        else:
                            print("   ⚠️  RPC getaddresshistory returned unexpected response format")
                            return False
                    else:
            ...
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=15), auth=self.auth) as session:
                async with session.post(url, json=payload) as response:
                    ...
                    if response.status == 200:
                        ...
                        # Check if the response contains an error
                        if 'error' in data:
                            print("   ❌ RPC getaddressbalance failed with error: {}".format(data['error']))
                            return False
                        elif 'result' in data:
                            print("   ✅ RPC getaddressbalance successful")
                            return True
                        else:
                            print("   ⚠️  RPC getaddressbalance returned unexpected response format")
                            return False
                    else:
