#!/usr/bin/env python3
"""
Bitcoin Protocol Implementation - Correct version message format
"""
...
class BitcoinProtocol:
    """Correct Bitcoin protocol implementation"""
    ...
    def create_version_message(self, peer_ip: str, peer_port: int) -> bytes:
        """Create a Bitcoin version message with correct format"""
        version = 70015
        services = 0
        ...
        user_agent = b'/BitcoinProtocol:0.1.0/'
        user_agent_bytes = struct.pack('<B', len(user_agent)) + user_agent
        start_height = 0
        relay = True
        
        # Create version payload with correct format
        # The correct format is: version(4) + services(8) + timestamp(8) + addr_recv_services(8) + addr_recv(26) + addr_trans_services(8) + addr_trans(26) + nonce(8)
        # But we need to be careful about the struct format string
...
def main():
    """Test Bitcoin protocol peer connections"""
    protocol = BitcoinProtocol()
    
    logger.info("Testing Bitcoin protocol peer connections...")
    success = protocol.test_peers()
    
    if success:
        logger.info("✅ Bitcoin protocol peer connection test successful!")
    else:
        logger.warning("❌ Bitcoin protocol peer connection test failed")
