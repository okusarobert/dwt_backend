...
from ..HD import ETH  # World Chain uses Ethereum-compatible addresses
from cryptography.fernet import Fernet
import base64


class AlchemyWorldConfig(BaseModel):
    """Pydantic model for Alchemy World Chain configuration"""
    api_key: str
    network: str = "mainnet"
    base_url: str = ""
    ws_url: str = ""
...
@dataclass
class WorldConfig:
    """Configuration for World Chain client"""
    api_key: str
    network: str = "mainnet"
    base_url: str = ""
    ws_url: str = ""
    ...
    @classmethod
    def testnet(cls, api_key: str) -> 'WorldConfig':
        return cls(
            api_key=api_key,
            network="testnet",
            base_url=f"https://world-testnet.g.alchemy.com/v2/{api_key}",
            ws_url=f"wss://world-testnet.g.alchemy.com/v2/{api_key}"
        )

    @classmethod
    def mainnet(cls, api_key: str) -> 'WorldConfig':
        return cls(
            api_key=api_key,
            network="mainnet",
            base_url=f"https://world-mainnet.g.alchemy.com/v2/{api_key}",
            ws_url=f"wss://world-mainnet.g.alchemy.com/v2/{api_key}"
        )
...
class WorldWallet:
    ...
    def __init__(self, user_id: int, config: WorldConfig, session: Session, logger: logging.Logger = None):
        self.user_id = user_id
        self.label = "World Chain Wallet"
    ...
    def create_wallet(self):
        """Create a new World Chain wallet account in the database."""
        try:
            ...
            self.logger.error(f"[World] Failed to create wallet for user {self.user_id}: {e!r}")
            self.logger.error(traceback.format_exc())
            raise  # Re-raise the exception so the wallet service can handle it

    def create_address(self):
        """Create a new World Chain address for the wallet with uniqueness guarantees."""
        try:
            ...
            if new_address:
                ...
                raise Exception("Failed to create unique World Chain address")
                
        except Exception as e:
            self.logger.error(f"[World] Failed to create address for user {self.user_id}: {e!r}")
            self.logger.error(traceback.format_exc())
            raise  # Re-raise the exception so the wallet service can handle it
        finally:
            self.logger.info(f"Done creating address for user {self.user_id}")

    # ===== World Chain API Methods =====
    
    def make_request(self, method: str, params: list = None) -> Optional[Dict]:
        """Make a JSON-RPC request to World Chain API"""
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
        ...
        try:
            response = self.session_request.post(
                self.world_config.base_url,
                json=payload,
                timeout=self.world_config.timeout
            )
    ...
    def test_connection(self) -> bool:
        """Test the connection to World Chain API"""
        self.logger.info("Testing World Chain API connection...")
    ...
    def get_balance(self, address: str) -> Optional[Dict]:
        ...
        if result and "result" in result:
            ...
            balance_wld = balance_wei / (10 ** 18)
            return {
                "address": address,
                "balance_wei": balance_wei,
                "balance_wld": balance_wld
            }
        return None
    ...
    def get_network_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "network": self.world_config.network,
                "latest_block": latest_block,
                "gas_price": gas_price,
                "base_url": self.world_config.base_url,
                "ws_url": self.world_config.ws_url
            }
        except Exception as e:
    ...
    def validate_address(self, address: str) -> bool:
        """Validate World Chain address format (same as Ethereum)"""
        try:
            # World Chain uses Ethereum addresses, so same validation
    ...
    def get_world_specific_info(self) -> Dict[str, Any]:
        """Get World Chain-specific information"""
        try:
            # Get World Chain specific information
            latest_block = self.get_latest_block_number()
            gas_price = self.get_gas_price()
            ...
            return {
                "network": "World Chain",
                "chain_id": 1000 if self.world_config.network == "mainnet" else 1001,  # World Chain mainnet/testnet
                "latest_block": latest_block,
                "gas_price": gas_price,
                ...
                "finality": "~1 second",
                "native_token": "WLD",
                "scaling_solution": "High-performance blockchain",
                "compatibility": "Ethereum-compatible"
            }
        except Exception as e:
            self.logger.error(f"Error getting World Chain-specific info: {e}")
            return {}
    ...
    def get_world_features(self) -> Dict[str, Any]:
        """Get World Chain-specific features"""
        try:
            return {
                "high_performance": True,
                "fast_finality": "~1 second",
                "high_throughput": "100,000+ TPS",
                "low_fees": True,
                "erc20_tokens": True,
            ...
            self.logger.error(f"Error getting World Chain features: {e}")
            return {}

    # ===== Address Uniqueness Methods =====

    def generate_new_address(self, index: int = None) -> Optional[Dict]:
        """Generate a new unique World Chain address for the wallet"""
        try:
            ...
            # World Chain uses Ethereum addresses, so we use the ETH HD wallet
            ...
            self.logger.error(f"Error generating new World Chain address: {e}")
            return None
            ...
            for addr_info in addresses:
                ...
                if balance:
                    address_balance = {
                        ...
                        "balance_wld": balance["balance_wld"],
                        "balance_wei": balance["balance_wei"]
                    }
                    address_balances.append(address_balance)
                    total_balance += balance["balance_wld"]
            
            return {
                "user_id": self.user_id,
                "account_id": self.account_id,
                "symbol": self.symbol,
                "total_balance_wld": total_balance,
                "addresses": addresses,
    ...
    def validate_address_uniqueness(self, address: str) -> Dict[str, Any]:
        ...
        try:
            # Check if address is valid World Chain format
