"""
Litecoin RPC Client for DWT Backend
Connects to remote Litecoin testnet node via RPC
"""
import json
import requests
import os
...
class LTCTransaction:
    """Litecoin transaction data structure"""
    txid: str
    amount: Decimal
    confirmations: int
    address: str
...
class LTCClient:
    """Litecoin RPC Client for testnet operations"""
    ...
    def _rpc_call(self, method: str, params: List[Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get('error'):
                raise Exception(f"Litecoin RPC error: {result['error']}")
                
            return result.get('result', {})
            
        except requests.exceptions.RequestException as e:
            logger.error(f"Litecoin RPC connection error: {e}")
            raise Exception(f"Failed to connect to Litecoin node: {e}")
        except json.JSONDecodeError as e:
            logger.error(f"Invalid JSON response from Litecoin node: {e}")
            raise Exception(f"Invalid response from Litecoin node: {e}")
    ...
    def litoshi_to_ltc(self, litoshis: int) -> Decimal:
        """Convert litoshis to LTC"""
        return Decimal(litoshis) / Decimal('100000000')
    
    def ltc_to_litoshi(self, ltc: Decimal) -> int:
        """Convert LTC to litoshis"""
        return int(ltc * Decimal('100000000'))

# Factory function for easy client creation
def create_ltc_client(host: str = None, port: int = None, 
                     username: str = None, password: str = None) -> LTCClient:
    """Create Litecoin client using environment variables"""
    host = host or os.getenv('LTC_TESTNET_RPC_HOST', 'localhost')
    port = port or int(os.getenv('LTC_TESTNET_RPC_PORT', '18333'))
    username = username or os.getenv('LTC_TESTNET_RPC_USER', 'user')
    password = password or os.getenv('LTC_TESTNET_RPC_PASSWORD', 'password')
    
    if not all([host, port, username, password]):
        raise ValueError("Missing required environment variables: LTC_TESTNET_RPC_HOST, LTC_TESTNET_RPC_PORT, LTC_TESTNET_RPC_USER, LTC_TESTNET_RPC_PASSWORD")
    
    return LTCClient(host, port, username, password, testnet=True)

# Example usage
if __name__ == "__main__":
    # Test the client
    client = create_ltc_client()
    
    try:
        # Test connection
        info = client.get_blockchain_info()
        print(f"Connected to Litecoin {info['chain']} network")
        print(f"Current block: {info['blocks']}")
        print(f"Headers: {info['headers']}")
        print(f"Sync progress: {client.get_sync_progress():.6f}%")
        ...
        print(f"Network version: {network.get('version', 'unknown')}")
