...
from ..HD import ETH  # Polygon uses Ethereum addresses
from cryptography.fernet import Fernet
import base64


class AlchemyPolygonConfig(BaseModel):
    """Pydantic model for Alchemy Polygon configuration"""
    api_key: str
    network: str = "mainnet"
    base_url: str = ""
    ws_url: str = ""
...
class PolygonConfig:
    ...
    @classmethod
    def testnet(cls, api_key: str) -> 'PolygonConfig':
        return cls(
            api_key=api_key,
            network="mumbai",
            base_url=f"https://polygon-mumbai.g.alchemy.com/v2/{api_key}",
            ws_url=f"wss://polygon-mumbai.g.alchemy.com/v2/{api_key}"
        )

    @classmethod
    def mainnet(cls, api_key: str) -> 'PolygonConfig':
        return cls(
            api_key=api_key,
            network="mainnet",
...
class PolygonWallet:
    ...
    def __init__(self, user_id: int, config: PolygonConfig, session: Session, logger: logging.Logger = None):
        self.user_id = user_id
        self.label = "Polygon Wallet"
    ...
    def create_wallet(self):
        """Create a new Polygon wallet account in the database."""
        try:
            ...
            self.logger.error(f"[Polygon] Failed to create wallet for user {self.user_id}: {e!r}")
            self.logger.error(traceback.format_exc())
            raise  # Re-raise the exception so the wallet service can handle it

    def create_address(self):
        """Create a new Polygon address for the wallet with uniqueness guarantees."""
        try:
            ...
            if new_address:
                ...
                raise Exception("Failed to create unique Polygon address")
                
        except Exception as e:
            self.logger.error(f"[Polygon] Failed to create address for user {self.user_id}: {e!r}")
            self.logger.error(traceback.format_exc())
            raise  # Re-raise the exception so the wallet service can handle it
        finally:
    ...
    def make_request(self, method: str, params: list = None) -> Optional[Dict]:
        """Make a JSON-RPC request to Polygon API"""
        payload = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": method,
        ...
        try:
            response = self.session_request.post(
                self.polygon_config.base_url,
                json=payload,
                timeout=self.polygon_config.timeout
            )
    ...
    def test_connection(self) -> bool:
        ...
        self.logger.info("Testing Polygon API connection...")
    ...
    def get_network_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "network": self.polygon_config.network,
                "latest_block": latest_block,
                "gas_price": gas_price,
                "base_url": self.polygon_config.base_url,
                "ws_url": self.polygon_config.ws_url
            }
        except Exception as e:
    ...
    def validate_address(self, address: str) -> bool:
        ...
        try:
            # Polygon uses Ethereum addresses, so same validation
    ...
    def get_polygon_specific_info(self) -> Dict[str, Any]:
        """Get Polygon-specific information"""
        try:
            # Get Polygon specific information
            latest_block = self.get_latest_block_number()
            gas_price = self.get_gas_price()
            ...
            return {
                "network": "Polygon",
                "chain_id": 137 if self.polygon_config.network == "mainnet" else 80001,  # Polygon mainnet/mumbai
                "latest_block": latest_block,
                "gas_price": gas_price,
                ...
                "finality": "~2 seconds",
                "native_token": "MATIC",
                "scaling_solution": "Layer 2 scaling for Ethereum",
                "compatibility": "Ethereum-compatible"
            }
        except Exception as e:
            self.logger.error(f"Error getting Polygon-specific info: {e}")
            return {}
    ...
    def get_polygon_features(self) -> Dict[str, Any]:
        """Get Polygon-specific features"""
        try:
            return {
                "high_performance": True,
                "fast_finality": "~2 seconds",
                "low_fees": True,
                "erc20_tokens": True,
                "smart_contracts": True,
                "ethereum_compatible": True,
                "layer2_scaling": True,
                "polygon_ecosystem": True
            }
        except Exception as e:
            self.logger.error(f"Error getting Polygon features: {e}")
            return {}

    # ===== Address Uniqueness Methods =====

    def generate_new_address(self, index: int = None) -> Optional[Dict]:
        """Generate a new unique Polygon address for the wallet"""
        try:
            ...
            # Polygon uses Ethereum addresses, so we use the ETH HD wallet
            ...
            self.logger.error(f"Error generating new Polygon address: {e}")
            return None
            ...
            for addr_info in addresses:
                ...
                if balance:
                    address_balance = {
                        ...
                        "balance_matic": balance["balance_matic"],
                        "balance_wei": balance["balance_wei"]
                    }
                    address_balances.append(address_balance)
                    total_balance += balance["balance_matic"]
            
            return {
                "user_id": self.user_id,
                "account_id": self.account_id,
                "symbol": self.symbol,
                "total_balance_matic": total_balance,
                "addresses": addresses,
    ...
    def validate_address_uniqueness(self, address: str) -> Dict[str, Any]:
        ...
        try:
            # Check if address is valid Polygon format
