...
class BaseDaemon:
    ...
    def parse_xpub(self, xpub):
        ...
        if isinstance(xpub, list):
            xpub_dict = xpub[0]
            return xpub_dict.pop("xpub", None), xpub_dict.pop("contract", None), xpub_dict
    ...
    async def get_handle_request_params(self, request):
        ...
        result = self.parse_xpub(kwargs.pop("xpub", None))

        if result is not None:
            xpub, contract, extra_params = result
        else:
            xpub, contract, extra_params = None, None, None
