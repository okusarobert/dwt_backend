...
# from web3.middleware import async_simple_cache_middleware  # Not available in newer web3.py versions
from web3.middleware.proof_of_authority import ExtraDataToPOAMiddleware
# from web3.providers.rpc import get_default_http_endpoint  # Not available in newer web3.py versions
...
async def async_http_retry_request_middleware(w3):
    async def middleware(make_request):
        async def retry_make_request(method, params):
            return await exception_retry_middleware(
                make_request,
                (AsyncClientError, TimeoutError, asyncio.TimeoutError, Web3Exception),
                daemon_ctx.get().VERBOSE,
            )(method, params)
        return retry_make_request
    return middleware
...
class ETHDaemon(BlockProcessorDaemon):
    ...
    async def create_coin(self):
        ...
        # web3.middleware_onion.inject(async_http_retry_request_middleware, layer=0)  # Not compatible with newer web3.py
        ...
        # web3.middleware_onion.add(async_simple_cache_middleware)  # Not available in newer web3.py versions
    ...
    def get_default_server_url(self):
        return "http://localhost:8545"  # Default Ethereum RPC endpoint
