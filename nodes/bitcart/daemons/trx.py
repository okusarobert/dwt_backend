...
class TRXDaemon(ETHDaemon):
    ...
    @rpc(requires_network=True)
    async def getaddresshistory(self, address, contract=None, network='testnet', wallet=None):
        ...
        url = f"https://api.{net}trongrid.io/v1/accounts/{address}/transactions"


        if contract is not None:
            url = f"{url}/trc20?contract_address={contract}&limit=200"
        else:
            url = f"{url}?limit=200"
        print(f"{url}")
    ...
    async def payto(self, destination, amount, fee=None, feerate=None, unsigned=False, wallet=None, *args, **kwargs):
        # print(f"WALLET to send {wallet} destination {destination}")
        address = self.wallets[wallet].address
        ...
        print(f"wallet key {self.wallets[wallet].keystore.private_key}")
