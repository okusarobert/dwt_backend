#!/usr/bin/env python3
"""
Optimism Client using Alchemy API
...
class OptimismClient:
    ...
    def __init__(self, api_key: str = None, network: str = "mainnet"):
        ...
        """
        self.api_key = api_key or os.getenv('ALCHEMY_OPTIMISM_API_KEY')
        self.network = network
        
        if not self.api_key:
            logger.warning("⚠️  No ALCHEMY_OPTIMISM_API_KEY environment variable found.")
        ...
        # Set up API URLs
        if network == "mainnet":
            self.api_url = f"https://opt-mainnet.g.alchemy.com/v2/{self.api_key}"
            self.ws_url = f"wss://opt-mainnet.g.alchemy.com/v2/{self.api_key}"
        else:
            self.api_url = f"https://opt-sepolia.g.alchemy.com/v2/{self.api_key}"
            self.ws_url = f"wss://opt-sepolia.g.alchemy.com/v2/{self.api_key}"
        
        logger.info(f"🔧 Initialized Optimism client for {network}")
        logger.info(f"📡 HTTP API URL: {self.api_url}")
        logger.info(f"🔌 WebSocket URL: {self.ws_url}")
    
    def make_request(self, method: str, params: list = None) -> Optional[Dict]:
        """Make JSON-RPC request to Optimism API"""
        try:
            payload = {
                "jsonrpc": "2.0",
                "id": 1,
...
class OptimismWebSocketClient:
    """Optimism WebSocket client for real-time monitoring"""
    
    def __init__(self, api_key: str = None, network: str = "mainnet"):
        """Initialize Optimism WebSocket client"""
        self.api_key = api_key or os.getenv('ALCHEMY_OPTIMISM_API_KEY')
        self.network = network
        self.ws = None
        ...
        logger.info(f"🔧 Initialized Optimism WebSocket client for {network}")
        logger.info(f"📡 WebSocket URL: {self.ws_url}")
    ...
    def on_open(self, ws):
        ...
        logger.info("📡 Subscribed to Optimism new blocks")
    ...
    def _handle_new_block(self, block_data):
        ...
        try:
            ...
            print("=" * 80)
            print(f"🆕 NEW OPTIMISM BLOCK #{block_number}")
            print("=" * 80)
    ...
    def start_monitoring(self, duration: int = 300):
        ...
        logger.info(f"🚀 Starting real-time Optimism block monitoring...")
        ...
        print("\n" + "="*80)
        print("🔧 REAL-TIME OPTIMISM BLOCK MONITOR")
        print("="*80)
...
def test_optimism_api():
    """Test Optimism API functionality"""
    print("🔧 Optimism API Test")
    print("📡 Using Alchemy Optimism API")
    print("=" * 50)
    
    # Check for API key
    api_key = os.getenv('ALCHEMY_OPTIMISM_API_KEY')
    if not api_key:
        print("⚠️  No ALCHEMY_OPTIMISM_API_KEY environment variable found.")
    ...
    # Create client
    client = OptimismClient(api_key=api_key, network="mainnet")
    ...
    # Test 2: Get balances for Optimism addresses
    print("\n🔍 Test 2: Getting balances for Optimism addresses")
    test_addresses = [
        {"name": "Optimism Foundation", "address": "0x0000000000000000000000000000000000000000"},
        {"name": "Optimism Treasury", "address": "0x0000000000000000000000000000000000000001"},
    ...
    usdc_address = "0x7F5c764cBc14f9669B88837ca1490cCa17c31607"  # USDC on Optimism
...
def main():
    ...
    client = OptimismWebSocketClient(api_key=api_key, network="mainnet")
